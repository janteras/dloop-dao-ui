...
var mockData = {
  ...
  protocolProposals: [
    ...
    },
    {
      ...
      proposer: "AI.Gov#01",
      createdAt: Math.floor(Date.now() / 1e3) - 1382400,
      updatedAt: Math.floor(Date.now() / 1e3) - 604800,
      forVotes: 78,
      againstVotes: 22,
      voteCount: 405320,
      executed: false,
      ...
      endTimestamp: Math.floor(Date.now() / 1e3) - 432e3,
      // 5 days ago in seconds
      endTimeISO: new Date(Date.now() - 432e3 * 1e3).toISOString(),
      // 5 days ago in ISO format
      endsAt: "Apr 25, 2024",
      details: [
    ...
    },
    {
      ...
      proposer: "0x7F5Ae2",
      createdAt: Math.floor(Date.now() / 1e3) - 2592e3,
      updatedAt: Math.floor(Date.now() / 1e3) - 864e3,
      forVotes: 92,
      againstVotes: 8,
      voteCount: 512680,
      ...
      endTimestamp: Math.floor(Date.now() / 1e3) - 864e3,
      // 10 days ago in seconds
      endTimeISO: new Date(Date.now() - 864e3 * 1e3).toISOString(),
      // 10 days ago in ISO format
      endsAt: "Apr 20, 2024",
      details: [
  ...
  delegations: [
    ...
    },
    {
      ...
      toType: "Human",
      amount: 200,
      date: Date.now() - 15 * 24 * 60 * 60 * 1e3
      // 15 days ago
    }
  ]
};
async function registerRoutes(app2) {
  app2.get("/api/proposals", (req, res) => {
    res.json(mockData.proposals);
  });
  app2.post("/api/proposals", (req, res) => {
    try {
      ...
      const sanitizedProposal = {
        ...newProposal,
        proposer: typeof newProposal.proposer === "string" ? newProposal.proposer : Array.isArray(newProposal.proposer) ? newProposal.proposer[0] : "Anonymous",
  ...
  app2.post("/api/proposals/prepare", (req, res) => {
  ...
  app2.get("/api/protocol/proposals", (req, res) => {
    res.json(mockData.protocolProposals);
  });
  app2.get("/api/protocol/metrics", (req, res) => {
    res.json(mockData.protocolMetrics);
  });
  app2.get("/api/ainodes", (req, res) => {
    const aiNodes = [
      {
        id: "node-1",
        name: "AI.Gov#01",
  ...
  app2.get("/api/ainodes/sentiments", (req, res) => {
    ...
    mockData.aiNodes.forEach((node) => {
      mockData.proposals.slice(0, 5).forEach((proposal) => {
        ...
        const baseSentiment = (nodeIdSum + proposal.id) % 200 - 100;
        const baseConfidence = 50 + nodeIdSum * proposal.id % 50;
        sentiments.push({
          nodeId: node.id,
  ...
  server: {
    proxy: {
      ...
      "/.netlify/functions/": {
        ...
        // Default Netlify Functions dev server
        changeOrigin: true,
        rewrite: (path3) => path3.replace(/^\/\.netlify\/functions/, ""),
