...
export function UndelegateTokensModal({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      amount: delegation.amount, // Default to full delegation amount for easier UX
    },
  });
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    // Extra validation for undelegation amount
    ...
    // Prevent zero amount undelegations
    if (data.amount <= 0) {
      toast({
        title: 'Invalid Amount',
    ...
    try {
      ...
      // Undelegate using our service
      await TokenDelegationService.undelegateTokens(
        signer,
        delegation.to,
      ...
      // Call onSuccess after successful delegation
      if (onSuccess) {
        onSuccess();
      }
      resetAndClose();
      ...
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <DialogHeader>
            ...
            <DialogDescription>
              Undelegate your DLOOP tokens from {displayName} ({delegation.toType}). This will increase your available balance and decrease their voting power.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={resetAndClose} className="mr-2">
              Cancel
            </Button>
            <Button 
              type="submit" 
            ...
            >
              {isSubmitting ? (
                <div className="flex items-center">
                  <span className="mr-2">Undelegating</span>
