...
import { extractVoteCounts, calculateVotingStats, formatVoteCount } from "@/utils/vote-helpers";
...
export function UnifiedProposalCard({ proposal, onRefresh, listView = false }: UnifiedProposalCardProps) {
  ...
  // Extract and calculate vote data using helper utilities
  const { forVotes, againstVotes } = extractVoteCounts(proposal);
  const { forPercentage, againstPercentage, totalVotes } = calculateVotingStats(forVotes, againstVotes);
          ...
          <div className="mb-4">
            <div className="flex justify-between mb-1 group/for">
              <p className="font-medium text-sm">
                For <span className="text-muted-foreground transition-colors duration-300 group-hover/for:text-green-500/80">({forPercentage > 0 ? Math.round(forPercentage) : 0}%)</span>
              </p>
              <p className="text-green-500 transition-all duration-300 group-hover/for:scale-110 group-hover/for:font-medium">
                {forVotes > 0 ? forVotes.toLocaleString() : '0'} votes
              </p>
            </div>
            <div className="h-2 mb-2 bg-secondary rounded-full overflow-hidden group/forbar transition-all duration-300 hover:h-3">
              <div 
                ...
                style={{ width: `${Math.min(Math.max(forPercentage, 0), 100)}%` }}
              ></div>
            </div>

            <div className="flex justify-between mb-1 group/against">
              <p className="font-medium text-sm">
                Against <span className="text-muted-foreground transition-colors duration-300 group-hover/against:text-red-500/80">({againstPercentage > 0 ? Math.round(againstPercentage) : 0}%)</span>
              </p>
              <p className="text-red-500 transition-all duration-300 group-hover/against:scale-110 group-hover/against:font-medium">
                {againstVotes > 0 ? againstVotes.toLocaleString() : '0'} votes
              </p>
            </div>
            <div className="h-2 bg-secondary rounded-full overflow-hidden group/againstbar transition-all duration-300 hover:h-3">
              <div 
                ...
                style={{ width: `${Math.min(Math.max(againstPercentage, 0), 100)}%` }}
              ></div>
            </div>
          </div>
        </div>
