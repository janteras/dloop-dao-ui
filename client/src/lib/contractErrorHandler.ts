...
export function handleContractError(error: any): ContractErrorResponse {
  ...
  if (error?.message?.includes('could not decode result data') && error?.value === '0x') {
    return {
      message: 'Contract not found on current network. Please ensure you are connected to Sepolia Testnet.',
      details: error.message,
      category: ContractErrorCategory.NETWORK_ERROR,
      retryable: true,
      suggestedAction: 'Please switch to Sepolia Testnet in your wallet and try again.'
    };
  }

  if (error?.code === 'NETWORK_ERROR' || error?.message?.includes('network')) {
    return {
      message: 'Network connection error. Please check your connection and ensure you are on Sepolia Testnet.',
      details: error.message,
      category: ContractErrorCategory.NETWORK_ERROR,
      retryable: true,
      suggestedAction: 'Check your network connection and switch to Sepolia Testnet.'
    };
  }
