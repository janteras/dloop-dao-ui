  ...
  const initWalletConnect = useCallback(async () => {
    try {
      ...
      provider.on('chainChanged', (newChainId: number) => {
        ...
        // Check if on Sepolia (chain ID 11155111 decimal = 0xaa36a7 hex)
        if (newChainId !== 11155111) {
          toast.error('Wrong network! Please switch to Sepolia Testnet');
          // Automatically attempt to switch if provider supports it
          if (provider.request) {
            provider.request({
              ...
              toast.warning('Please manually switch to Sepolia Testnet');
            });
          }
        } else {
    ...
    try {
      ...
      if (accounts && accounts.length > 0) {
        ...
        if (currentChainId !== 11155111) {
          toast.warning('Connected but wrong network. Please switch to Sepolia Testnet');
          ...
          toast.success('Connected to Sepolia Testnet via WalletConnect');
        }
      } else {
