...
import { soulboundNftService } from './soulboundNftService';
...
class AINodeIntegrationService {
  ...
  async getAllAINodes(provider: ethers.BrowserProvider | null): Promise<AINode[]> {
    try {
      ...
      // Using getAllNodes instead of getAllAINodes to match the mockAiNodeService interface
      const aiNodes = await mockAiNodeService.getAllNodes();
      ...
      try {
        ...
        try {
          ...
          const enrichedNodes = aiNodes.map((node: AINode) => {
            // Find matching NFT for this node based on Node ID attribute
            const matchingNft = nftTokens.find((nft: StructuredToken) => 
              nft.metadata && 
          ...
          console.log(`Successfully enriched AI nodes with NFT data: ${enrichedNodes.filter((n: AINode) => n.soulboundTokenId).length} nodes have NFTs`);
  ...
  async getAINodeById(nodeId: string, provider: ethers.BrowserProvider | null): Promise<AINode | null> {
    try {
      ...
      // Using getNodeById instead of getAINodeById to match the mockAiNodeService interface
      const node = await mockAiNodeService.getNodeById(nodeId);
