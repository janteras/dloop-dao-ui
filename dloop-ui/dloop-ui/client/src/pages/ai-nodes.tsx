...
import { ExternalLink, BarChart3, ShieldCheck, Brain, ServerIcon, Network, Tag } from 'lucide-react';
import AINodeExplorer from "@/components/features/ai-nodes/ai-node-explorer-fixed";
import AINodeNFTExplorer from "@/components/features/ai-nodes/ai-node-nft-explorer";
import AINodeNFTDetail from "@/components/features/ai-nodes/ai-node-nft-detail";
...
// Enhanced AI Nodes page that incorporates SoulboundNFT integration
export default function AINodesPage() {
  const [location] = useLocation();
  const { isConnected } = useWallet();
  
  // Check if we're on a node or NFT detail page
  const isNodeDetail = location.startsWith('/ai-nodes/') && location.split('/').length === 3 && !location.includes('/nft/');
  const isNFTDetail = location.startsWith('/ai-nodes/nft/') && location.split('/').length === 4;
  ...
  // Handle NFT explorer view
  if (location === '/ai-nodes/nft-explorer') {
    return (
      <PageContainer>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold">AI Node NFTs</h1>
            <p className="text-muted-foreground mt-1">
              Explore all 10 Soulbound NFTs for AI Governance Nodes
            </p>
          </div>
          <Button 
            onClick={() => window.open("https://sepolia.etherscan.io/token/0x6391C14631b2Be5374297fA3110687b80233104c", "_blank")}
      ...
      >
        <TabsList className="mb-6">
          ...
          <TabsTrigger value="nfts">
            <Tag className="h-4 w-4 mr-2" /> NFT Collection
          </TabsTrigger>
          <TabsTrigger value="analytics">
            <BarChart3 className="h-4 w-4 mr-2" /> View Performance
          </TabsTrigger>
          <TabsTrigger value="classic">
            <ExternalLink className="h-4 w-4 mr-2" /> Legacy Interface
          </TabsTrigger>
        </TabsList>
