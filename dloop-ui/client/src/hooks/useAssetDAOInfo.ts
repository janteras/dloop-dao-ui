    ...
    async function fetchAssetDAOInfo() {
      ...
      try {
        ...
        try {
          ...
          // First try the governanceToken() function
          const tokenAddress = await assetDaoContract.governanceToken().catch(async (error) => {
            console.warn('governanceToken() failed, trying alternative method...', error);
            // If that fails, try an alternative method (check if token is stored in a different way)
            // Many contracts have a standard token() function instead
            try {
              return await assetDaoContract.token();
            } catch (altError) {
              console.warn('Alternative token() method also failed:', altError);
              // If both methods fail, fallback to the DLoopToken address from config
              const { ADDRESSES } = await import('@/config/contracts');
              return ADDRESSES.DLoopToken;
            }
          });
