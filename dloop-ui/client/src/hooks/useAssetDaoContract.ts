...
function mapProposalFromContract(proposal: any, proposalState?: number): ProposalDetails {
  ...
  return {
    ...
    token: proposal.token as string,
    amount: amountNumber,
    forVotes: proposal.forVotes ? Number(formatEther(proposal.forVotes.toString())) : 0,
    againstVotes: proposal.againstVotes ? Number(formatEther(proposal.againstVotes.toString())) : 0,
