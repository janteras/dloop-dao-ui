...
export function CreateProtocolProposalModal({ isOpen, onClose }: CreateProtocolProposalModalProps) {
    ...
    try {
      ...
      if (signer) {
        ...
        if (data.proposalType === "parameter-update") {
          ...
          // Create function selector (4 bytes) - in a real implementation, we'd compute this
          // Here we'll use a direct encoding approach
          const encodedFunctionCall = encodedParams;
          ...
          const encodedFunctionCall = ethers.AbiCoder.defaultAbiCoder().encode(
            ["address", "bool"],
            [data.tokenAddress, data.isWhitelisted]
          );
          
          // Add function selector + encoded params
          calldatas = [encodedFunctionCall];
          ...
          // Create function selector + encoded params
          const encodedFunctionCall = encodedParams;
          calldatas = [encodedFunctionCall];
        ...
        try {
          const tx = await protocolDaoContract.createProposal(
            description,
            targets,
            values.map(v => ethers.parseEther(v)), // Convert string values to BigNumbers
            calldatas,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="bg-dark-gray text-white border-gray max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-white">Create Protocol Proposal</DialogTitle>
          <DialogDescription className="text-gray">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray">Proposal Title</FormLabel>
                  <FormControl>
                    <Input
                      {...field}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray">Proposal Description</FormLabel>
                  <FormControl>
                    <Textarea
                      {...field}
                      className="bg-dark-bg border-gray text-white focus-visible:ring-accent min-h-32 resize-y"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray">Proposal Type</FormLabel>
                  <Select
                    onValueChange={field.onChange}
                    defaultValue={field.value}
                  >
                    <FormControl>
                      <SelectTrigger className="bg-dark-bg border-gray text-white focus:ring-accent">
                        <SelectValue placeholder="Select proposal type" />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent className="bg-dark-gray border-gray text-white">
            ...
            {proposalType === "parameter-update" && (
              <>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">Parameter Type</FormLabel>
                      <Select
                        onValueChange={field.onChange}
                        defaultValue={field.value}
                      >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">Parameter Value</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          className="bg-dark-bg border-gray text-white focus-visible:ring-accent"
            ...
            {proposalType === "token-whitelist" && (
              <>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">Token Contract Address</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          className="bg-dark-bg border-gray text-white focus-visible:ring-accent font-mono"
                          placeholder="0x..."
                        />
                      </FormControl>
                      <FormDescription className="text-gray-400">
                ...
                <FormField
                  ...
                  name="isWhitelisted"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border border-gray p-4">
                      <FormControl>
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            className="rounded border-gray bg-dark-bg focus:ring-accent h-4 w-4 text-accent"
                          />
                          <span className="text-gray-300">Add token to whitelist</span>
                        </div>
                      </FormControl>
                      <FormDescription className="text-gray-400 text-sm mt-1 ml-6">
            ...
            {proposalType === "role-management" && (
              <>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">Role Type</FormLabel>
                      <Select
                        onValueChange={field.onChange}
                        defaultValue={field.value}
                      >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">New Address</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          className="bg-dark-bg border-gray text-white focus-visible:ring-accent font-mono"
            ...
            {proposalType === "custom-execution" && (
              <>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">Target Addresses</FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
                          className="bg-dark-bg border-gray text-white focus-visible:ring-accent font-mono"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">ETH Values</FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
                          className="bg-dark-bg border-gray text-white focus-visible:ring-accent font-mono"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray">Calldata</FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
                          className="bg-dark-bg border-gray text-white focus-visible:ring-accent font-mono"
            ...
            <DialogFooter className="gap-2">
              <Button
                ...
                variant="outline"
                onClick={handleClose}
                className="bg-dark-bg border-gray text-white hover:bg-accent/10"
              >
                Cancel
              </Button>
              <Button
